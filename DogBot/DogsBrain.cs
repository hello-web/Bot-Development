...
namespace DogsBrain
{
    public class DogsMind
    {
        ...
        public DogsMind(ALIVE.SmartDog dog/*, SpeechSynthesizer myS, SpeechRecognitionEngine myR, sgrMachine pm*/)
        {
            myDog = dog;
/*            mySynth = myS;
            myRecog = myR;*/     // Wei Chen 2010-04-20
            min_x = 90;
            min_y = 90;
            max_x = 160;
            max_y = 160;
...
/*                if (fam == "" || concept.all_concepts[fam] == null)
        ...
        public bool unexplored(out int xi, out int yi)
        {
            float x, y;
            int lim;
            myDog.Coordinates(out x, out y);
            lim = max_x - min_x + max_y - min_y;
            bool res = dogTricks.find_unexplored_rel(this, x, y, out xi, out yi, lim);
            if (res) Console.WriteLine("Found an unexplored point nearest to the dog at: <" + xi.ToString() + "," + yi.ToString() + ">");
            return res;
        }
